                ...
                <div className="grid grid-cols-5 gap-[1vw] max-sm:flex max-sm:gap-[4vw] max-sm:flex-nowrap max-sm:w-fit max-sm:px-[5vw] max-md:flex max-md:flex-wrap max-md:justify-center max-md:gap-[3vw]">
                    {logos.map((logo, index) => (
                        <div
                            ...
                            key={index}
                            className={`rounded-4xl w-[13.35vw] h-[13.35vw] first:col-start-2 nth-4:col-start-1 border border-gray-500 group hover:bg-[var(--hover-color)] hover:border-transparent duration-300 ease-out max-sm:h-[35vw] max-sm:w-[35vw] max-md:h-[20vw] max-md:w-[20vw]`}>
                            <Image src={logo.src} alt="montra network" width={200} height={200} className="w-full h-full duration-300 group-hover:invert-0 max-sm:invert-0 invert" />
                        </div>
                    ))}
                </div>
                </div>
