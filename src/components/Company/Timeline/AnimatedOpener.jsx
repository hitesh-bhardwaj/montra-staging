...
const AnimatedOpeners = ({ letter, classPrefix, left, top, color, title, z, content }) => {
  ...
  return (
    <span className={`relative ${left} ${top} ${classPrefix} ${z} `}>
      <div
        className={`w-[20vw] rounded-[3vw] absolute overflow-hidden`}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <div className={`w-[20vw] h-[4.2vw] border-[2px] relative border-black overflow-hidden rounded-[3vw]  circle ${classPrefix}-circle`}>
          <div className={`w-fit bg-black rounded-[3vw] flex h-full z-[-1] ${classPrefix}-container`}>
            <span
              style={{ backgroundColor: color }}
              className={` h-[4vw] w-[4vw] rounded-full flex justify-center items-center`} />
            <div
              style={{ backgroundColor: color }}
              className={`h-full rounded-[3vw] w-[20vw] text-black-1 px-[2vw] flex flex-col items-start py-[1vw] capitalize relative gap-[1vw]`}>
              <div className="h-[3vw] ">
                {title}
              </div>
              ...
              <div className={`text-[1vw] ${classPrefix}-content font-normal`}>
