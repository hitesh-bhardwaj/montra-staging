...
//     //       const slideSelector = `#slide-${i + 1}`;

//     //       // build one timeline per card:
...
//     //       // 1) fade in
//     //       tl.fromTo(slideSelector,
//     //         { opacity: 0, y: 50 },
//     //         { opacity: 1, y: 0, duration: 0.2 }
//     //       )
//     //         // 2) pause briefly in the “visible” state
//     //         .to({}, { duration: 0.1 })
//     //         // 3) fade out
//     //         .to(slideSelector,
...
//         const endScroll = (foc_sections.length - 1) * window.innerHeight;
//         const cards = gsap.utils.toArray('.useCase-item');
//         const cardsContent = gsap.utils.toArray('.slides-content');

//         // initialize all cards hidden
//         gsap.set(cardsContent, { opacity: 0, y: 50 });
...
//             },
//             markers: true,
//             onEnter: () => gsap.to(`#slide-${i + 1}`, { opacity: 1, duration: 0.5, y: 0, z: 50, delay: 0.2 }),
//             onLeave: () => gsap.to(`#slide-${i + 1}`, { opacity: 0, duration: 0.5, y: -50, z: 1 }),
//             onEnterBack: () => gsap.to(`#slide-${i + 1}`, { opacity: 1, duration: 0.5, y: 0, z: 50, delay: 0.2 }),
//             onLeaveBack: () => gsap.to(`#slide-${i + 1}`, { opacity: 0, duration: 0.5, y: 50, z: 1 }),
//           });
//         });
//       });
...
//   return (
//     <>
//       <section id="use-cases" className={`w-screen relative ${mode === "personal" ? "h-[500vh]" : "h-[700vh]"}`}>

//         <div className="w-screen h-screen sticky top-0 left-0">
//           {/* Mode Toggle */}
//           <div className="absolute top-[8%] z-[100] left-1/2 -translate-x-1/2 flex gap-2 bg-white border border-[#E2EFFF] rounded-full p-0.5 min-w-[220px]">
//             <div ref={sliderRef} className="absolute top-[6%] left-0 w-1/2 h-[88%] rounded-full bg-primary" />
//             <button onClick={() => swapMode("personal")} className={`relative z-10 px-[1.2vw] py-[0.5vw] rounded-full text-[1vw] transition-colors duration-200 ${mode === "personal" ? "text-white" : "text-black"}`} >
//               Personal
//             </button>
//             <button onClick={() => swapMode("business")} className={`relative z-10 px-[1.2vw] py-[0.5vw] rounded-full text-[1vw] transition-colors duration-200 ${mode === "business" ? "text-white" : "text-black"}`} >
//               Business
//             </button>
//           </div>

//           {/* Navigation */}
//           <div className="top-1/2 left-[3%] absolute -translate-y-1/2 w-fit flex gap-3 items-start justify-start z-[100]">
//             {/* SVG timeline */}
//             <div className="relative h-[15vw] w-[20px]">
//               <svg
...
//                 id="progress-svg"
//               >
//                 <line x1="6.95801" y1="5.5" x2="6.958" y2="257.5" stroke="#EEEEEE" strokeWidth="3" />
//                 <line
//                   id="progress-line"
//                   x1="6.95801"
//                   y1="5.5"
...
//             <ul className="use-cases-list flex flex-col gap-[1.1vw] -mt-[0.7vw] h-full">
...
//           {/* Skip Button */}
//           <button onClick={handleSkip} className="absolute bottom-10 right-10 z-[100] text-[1vw] font-display flex items-center gap-1 group hover:text-primary cursor-pointer" >
//             Skip
...
//           {/* Content Block */}
//           <div className="absolute left-[20%] h-screen w-[40vw] z-[50]">
//             {stepsData[mode].map((item, index) => (
//               <div className="absolute w-full h-screen flex flex-col justify-center top-0 left-0 slides-content" id={`slide-${item.id}`} key={index}>
...
//           {/* Images Container */}
//           <div className="absolute top-0 right-0 w-2/5 h-screen flex items-center justify-center">
//             <div className="relative w-full h-[70vh] overflow-hidden fadeupanim">
//               <Image className="absolute inset-0 object-contain top-0 left-0 h-full w-full z-30" alt="mobile frame" src="/assets/animation/frame.png" width={500} height={800} quality={100} />
//               {stepsData[mode].map((step, idx) => (
//                 <React.Fragment key={idx}>
//                   <div
//                     className="absolute top-0 left-0 h-full w-full frame-image-wrapper opacity-0 translate-x-full"
//                     style={{ clipPath: 'rect(0% 100% 100% 28% round 5vw)' }}
//                     data-index={idx}
//                   >
//                     <Image
...
//                         height={750}
//                         quality={100}
//                         className="w-full h-full absolute top-0 left-0 object-contain opacity-0"
//                       />
//                     </div>
//                   )}
//                 </React.Fragment>
...
//         <div className="w-screen h-full mt-[-100vh] pointer-events-none">
//           <div className="w-[50%] ml-[15%] h-full relative flex flex-col">
//             {stepsData[mode].map((item, index) => (
//               <div className="useCase-item w-full h-screen pt-[10%] !static flex items-center" id={item.id} key={index}>
//               </div>
//             ))}
//           </div>
//         </div>
  ...
  if (globalThis.innerWidth > 1024) {
    useEffect(() => {
      let ctx = gsap.context(() => {
        ...
        // 4) for each section, fade its content in then out within its “slice” of the timeline
        sections.forEach((section, i) => {
          const content = section.querySelector('.usecase-item');

          // fade-in over the first half of this section’s scroll, starting at time = i
          scrollTween.fromTo(
            content,
            { opacity: 0, y: 50 },
          ...
          // then fade-out over the last half, starting at time = i + 0.5
          scrollTween.to(
            content,
        ...
        <div className="content">
          ...
          <div id="useCase-Scroll">
            <div className="useCases-Container w-full overflow-hidden" id="panels">
              ...
              <div className="fadeUp flex w-full h-screen" id="panels-container">
